...
class Model(ModelDesc):
	...
	# (top1, index) = tf.nn.top_k(softmaxout, k=1, name='outputtop1')
        ...
        self.cost = tf.add_n([wd_cost], name='cost')
...
def eval_on_ILSVRC12(model_file, data_dir, outfile):
    ds, imglist = get_data('val')
    pred_config = PredictConfig(
        model=Model(),
        session_init=get_model_loader(model_file),
        input_names=['input', 'label'],
        # output_names=['softmaxout', 'outputtop1', 'label', 'input']
        output_names=['feat' ]
	...
	for i in range(batch_size):
		feat =  o[0][i] 
		imgpath = imglist[count][0]
		# print count
		count += 1
		# print feat.shape, imgpath
		# print feat
		# save to path
		outfile = data_dir + '/' + imgpath + '.bin'
		f = open(outfile, 'wb')
		feat.tofile(f)
		f.close()
